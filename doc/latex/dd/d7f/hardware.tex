...
For the software description of the \doxylink{namespace_robohat}{Robohat}, see \doxysectlink{software}{Software description}{0} ~\newline
...
\doxysection*{\doxylink{namespace_robohat}{Robohat} hardware description}



The \doxylink{namespace_robohat}{Robohat} is Robot which has a modular design. The basis of the design is the brain. The brain consist of PCBs and batteries.~\newline
...
For the processing power, the \doxylink{namespace_robohat}{Robohat} uses a Raspberry PI 4~\newline
...
The maximum amount of servos the user can connect to the \doxylink{namespace_robohat}{Robohat} hardware is 32 (but can be limited if the servos consumes much power).~\newline
 These servos will be driven by the Servo-\/assemblies. For how to connect a servo to the \doxylink{namespace_robohat}{Robohat}, see \textquotesingle{}Servo-\/assembly\textquotesingle{}
...
A Servo assembly is the connection between the servos and the rest of the \doxylink{namespace_robohat}{Robohat}.



The \doxylink{namespace_robohat}{Robohat} can hold 2 Servo assemblies.
...
The Servo assembly handles the power distribution of the servos. Each Servo assembly has to 16 servo channels. Those servo-\/channels are grouped in 4 groups.~\newline
 Each group has its own powersupply. The current of each power supply is limited to 5 Ampère. ~\newline
...
If needed, he \doxylink{namespace_robohat}{Robohat} has room for 2 batteries. But 1 battery should be enough.~\newline
...
At the top-\/side there a multiple connectors, which handles the power delivery for the \doxylink{namespace_robohat}{Robohat}.
...
At the back-\/side are the connectors for turning he \doxylink{namespace_robohat}{Robohat} on and off.
...
{\bfseries{Turning on the \doxylink{namespace_robohat}{Robohat}}}

User Power on~\newline
 The \doxylink{namespace_robohat}{Robohat} can only be turned on when the accu has enough capacity. To power on, the user has to press the switch of the switch-\/board.~\newline
 ...
 Nothing will happen, when the user releases the button before flashing starts or after flashing is finished. This window of releasing the button is done, to prevent accidentally turning on the \doxylink{namespace_robohat}{Robohat}. When the power is on, the green power-\/\+LED will be illuminated.

User Powering off~\newline
 Powering down should be done with the shutdown API-\/function of the \doxylink{namespace_robohat}{Robohat} library.~\newline
 The user can also shut down the OS (by \textquotesingle{}sudo shutdown now\textquotesingle{} in the console), and turn off the power by pressing switch the same way as turning the \doxylink{namespace_robohat}{Robohat} on. (When the switch is pressed the red status-\/\+LED will blink 3 times. When the blinking period ends, the red status-\/\+LED will flash multiple times. The user should release the power switch before the flashes stops).
...
When using the \doxylink{namespace_robohat}{Robohat} lib, the user will get a first warning when the battery capacity goes below 20 \%. A second warning will be presented when the capacity of the battery goes down below 10\%. When the \doxylink{namespace_robohat}{Robohat} lib detects a capacity below 5\%, an auto shutdown will be performed to save the accu. This will be a graceful shutdown. The OS will be shut downed before the power is shut downed.

While the accu is in standby, the LED will periodically flash (interval of 6 seconds) to see the battery is still attached to the \doxylink{namespace_robohat}{Robohat}.~\newline
 Its better to unplug the battery and store it in a safe place, when the \doxylink{namespace_robohat}{Robohat} os not used.
...
The \doxylink{namespace_robohat}{Robohat} has also a \textquotesingle{}Inertial Measurement Unit\textquotesingle{} (IMU for short). With the IMU, the \doxylink{namespace_robohat}{Robohat} can track its movements.~\newline
 It can determine its position by reading out the X,Y and Z magnetic fields direction. Also, it can measure the acceleration in the X, Y en Z direction. It has also a build in Gyroscope, so the \doxylink{namespace_robohat}{Robohat} can know when it turns.~\newline
...
Below is a picture how the \doxylink{namespace_robohat}{Robohat} PCBs are connected together. The Second assembly is not mandatory. There is no need for the second accu, and the second PWM management board, if the second assembly board is not connected.
