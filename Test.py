...
class Example:
    ...
    def do_scan_servos(self) -> None:
        ...
        if (
            self.__robohat.get_assemblyboard_is_connected(PwmPlug.PWMPLUG_P3) == False
            and self.__robohat.get_assemblyboard_is_connected(PwmPlug.PWMPLUG_P4)
            == False
        ):
            ...
            if sub_command == "angle" and len(data_in_list) == 5:
                ...
                if io_command == "dir":
                    ...
                    if value == "OUT" or value == "out":
                        stat = self.__robohat.set_servo_io_expander_direction(
                            pwm_plug, pin_nr, ExpanderDir.OUTPUT
                        )
                        if stat == IOStatus.IO_OK:
                            print("Direction set to output")
                    elif value == "IN" or value == "in":
                        stat = self.__robohat.set_servo_io_expander_direction(
                            pwm_plug, pin_nr, ExpanderDir.INPUT
                        )
                        if stat == IOStatus.IO_OK:
                    ...
                    if value == "HIGH" or value == "high":
                        stat = self.__robohat.set_servo_io_expander_output(
                            pwm_plug, pin_nr, ExpanderStatus.HIGH
                        )
                        if stat == IOStatus.IO_OK:
                            print("Pin set to HIGH")
                    elif value == "LOW" or value == "low":
                        stat = self.__robohat.set_servo_io_expander_output(
                            pwm_plug, pin_nr, ExpanderStatus.LOW
                        )
                        if stat == IOStatus.IO_OK:
        ...
        # ------------------------------------------------------------------------------
            ...
            if sub_command == "io":
                ...
                if io_command == "dir":
                    ...
                    if value == "OUT" or value == "out":
                        stat = self.__robohat.set_topboard_io_expander_direction(
                            pin_nr, ExpanderDir.OUTPUT
                        )
                        if stat == IOStatus.IO_OK:
                            print("Direction set to output")
                    elif value == "IN" or value == "in":
                        stat = self.__robohat.set_topboard_io_expander_direction(
                            pin_nr, ExpanderDir.INPUT
                        )
                        if stat == IOStatus.IO_OK:
                            print("Direction set to input")
                    else:
                    ...
                    value = data_in_list[5]
                    if value == "HIGH" or value == "high":
                        stat = self.__robohat.set_topboard_io_expander_output(
                            pin_nr, ExpanderStatus.HIGH
                        )
                        if stat == IOStatus.IO_OK:
                            print("Pin set to HIGH")
                    elif value == "LOW" or value == "low":
    ...
    def process_get(self, _data_in: str) -> None:
        ...
        if command == "servo" and len(data_in_list) >= 3:
            ...
            if sub_command == "angle" and len(data_in_list) == 4:
                ...
                if parameter_str.isnumeric():
                    ...
                    if value != -1:
                        print(
                            "adc of servo " + str(servo_nr) + " is: " + str(value) + "V"
                        )
                elif parameter_str == "all":
                ...
                if io_command == "dir":
                    ...
                    pin_nr = int(data_in_list[5])
                    value = self.__robohat.get_servo_io_expander_direction(
                        pwm_plug, pin_nr
                    )
                    print(value)
        ...
        # -------------------------------------------------------------------------------
            ...
            if sub_command == "builddate":
                print(
                    "build date of Robohat lib is: "
                    + self.__robohat.get_lib_build_date()
                )
